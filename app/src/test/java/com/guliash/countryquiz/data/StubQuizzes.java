package com.guliash.countryquiz.data;

import com.guliash.countryquiz.quiz.model.Quiz;

import edu.emory.mathcs.backport.java.util.Arrays;

public class StubQuizzes {

    public static final Quiz QUIZ1 = new Quiz("1", "Great Sphinx of Giza", "https://en.wikipedia.org/wiki/Great_Sphinx_of_Giza",
            "statue", Arrays.asList(new String[]{"Egypt", "Russia", "USA", "China"}),
            "Egypt",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/d/d3/Great_Sphinx_of_Giza_May_2015.JPG/1024px-Great_Sphinx_of_Giza_May_2015.JPG",
            "By MusikAnimal - Own work, CC BY-SA 3.0, https://commons.wikimedia.org/w/index.php?curid=40954313");

    public static final Quiz QUIZ2 = new Quiz("2", "Leshan Giant Buddha", "https://en.wikipedia.org/wiki/Leshan_Giant_Buddha",
            "statue", Arrays.asList(new String[] {"Vietnam", "Thailand", "China", "India"}), "China",
            "https://upload.wikimedia.org/wikipedia/commons/thumb/3/38/Leshan_Buddha_Statue_View.JPG/800px-Leshan_Buddha_Statue_View.JPG",
            "By Ariel Steiner - Own work, CC BY-SA 2.5, https://commons.wikimedia.org/w/index.php?curid=1662053");

}
